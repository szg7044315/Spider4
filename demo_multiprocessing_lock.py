...
class LockProcess(Process):
    ...
    def __init__(self, name=None, lock=None, m_count=0):
    ...
    def run(self):
        self.lock.acquire()
        print('process is ' + multiprocessing.current_process().name + ' process lock acquire')
        # 性能问题，100000次循环，所以这里优化为先从多进程共享拿出来计算完再放回多进程共享
        count = self.m_count.value
        for i in range(0, 100000):
            count += 1
        self.m_count.value = count
        print('process is ' + multiprocessing.current_process().name + ' process lock release')
        self.lock.release()
